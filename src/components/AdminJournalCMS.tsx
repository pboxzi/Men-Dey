    ...
    <div className="space-y-6 text-left">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[rgba(0,0,0,0.06)] pb-4">
        <div>
          ...
          <p className="text-xs text-[#444] font-mono">Manage journal entries and articles.</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          { label: 'Entries', value: entries.length, icon: FileText, color: 'text-[#C89B3C]' },
      ...
      {loading ? (
        ...
        filteredEntries.length === 0 ? (
          <div className="rounded-xl border border-[rgba(0,0,0,0.06)] p-12 text-center space-y-2">
            ...
            <p className="text-sm text-[#444]">No entries found.</p>
          </div>
        ) : (
              ...
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="divide-y divide-neutral-900/40">
                  {filteredEntries.map(entry => (
                    <tr key={entry.id as string} className="hover:bg-neutral-100 transition-all">
                      ...
                      <td className="px-3 py-3">
                        {entry.category ? (
                          <span className="px-1.5 py-0.5 rounded bg-[#C89B3C]/10 border border-[#C89B3C]/20 text-[10px] font-mono text-[#C89B3C] uppercase">{entry.category as string}</span>
                        ) : (
                          <span className="text-[10px] font-mono text-[#444]">-</span>
                        )}
                      </td>
                      <td className="px-3 py-3 font-mono text-[#444] text-[10px]">{(entry.date as string) || '-'}</td>
                      <td className="px-3 py-3">
                        {entry.read_time ? (
                          <span className="inline-flex items-center gap-1 text-[10px] font-mono text-[#444]">
                      ...
                      <td className="px-3 py-3">
                        <p className="text-[10px] text-[#444] line-clamp-2 max-w-[200px]">{(entry.excerpt as string) || '-'}</p>
                      </td>
        ...
        filteredArticles.length === 0 ? (
          <div className="rounded-xl border border-[rgba(0,0,0,0.06)] p-12 text-center space-y-2">
            <FileText className="h-8 w-8 text-[#444] mx-auto" />
            <p className="text-sm text-[#444]">No articles found.</p>
          </div>
        ) : (
          <div className="rounded-xl border border-[rgba(0,0,0,0.06)] bg-[#F8F6F2] overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="divide-y divide-neutral-900/40">
                  {filteredArticles.map(article => (
                    <tr key={article.id as string} className="hover:bg-neutral-100 transition-all">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <div className="h-7 w-7 rounded bg-white border border-[rgba(0,0,0,0.06)] flex items-center justify-center shrink-0">
                      ...
                      <td className="px-3 py-3 font-mono text-[10px] text-[#444]">{(article.slug as string) || '-'}</td>
                      <td className="px-3 py-3">
                      ...
                      <td className="px-3 py-3">
                        {article.is_featured ? (
                          <span className="flex items-center gap-1 text-[10px] font-mono text-[#C89B3C]">
                      ...
                      <td className="px-3 py-3">
                        <span className="px-1.5 py-0.5 rounded bg-white border border-[rgba(0,0,0,0.06)] text-[10px] font-mono text-[#444] uppercase">
                          {catName(article.category_id as string | null)}
                        </span>
                      </td>
                      <td className="px-3 py-3 text-[11px] text-[#444]">{(article.author as string) || '-'}</td>
                      <td className="px-3 py-3 text-center">
                        <div className="flex items-center justify-center gap-1">
      ...
      <AnimatePresence>
        {editModal && editModal.table === 'journal_entries' && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/80 backdrop-blur-sm" onClick={() => setEditModal(null)}>
            ...
            >
              <div className="px-5 py-3.5 border-b border-[rgba(0,0,0,0.06)] flex items-center justify-between">
                ...
                <button onClick={() => setEditModal(null)} className="p-1 rounded text-[#444] hover:text-white" aria-label="Close"><X className="h-4 w-4" /></button>
              </div>
              ...
              <form onSubmit={saveEntry} className="p-5 space-y-4 max-h-[75vh] overflow-y-auto">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-mono text-[#444] uppercase tracking-wider">Date</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-mono text-[#444] uppercase tracking-wider">Read Time</label>
                    <input type="text" value={entryReadTime} onChange={e => setEntryReadTime(e.target.value)} placeholder="e.g. 5 min read"
                      className="w-full bg-white border border-[rgba(0,0,0,0.06)] rounded px-3 py-2 text-xs text-white outline-none focus:border-[#C89B3C]/40" />
                  </div>
                </div>
                <div className="space-y-1.5">
                  <label className="text-[11px] font-mono text-[#444] uppercase tracking-wider">Excerpt</label>
                ...
                <div className="space-y-1.5">
                  <label className="text-[11px] font-mono text-[#444] uppercase tracking-wider">Content</label>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/80 backdrop-blur-sm" onClick={() => setEditModal(null)}>
            ...
            >
              ...
              <form onSubmit={saveArticle} className="p-5 space-y-4 max-h-[75vh] overflow-y-auto">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-mono text-[#444] uppercase tracking-wider">Slug</label>
                ...
                <div className="space-y-1.5">
                  <label className="text-[11px] font-mono text-[#444] uppercase tracking-wider">Cover Image URL</label>
                  <input type="text" value={articleCoverImageUrl} onChange={e => setArticleCoverImageUrl(e.target.value)} placeholder="https://..."
                    className="w-full bg-white border border-[rgba(0,0,0,0.06)] rounded px-3 py-2 text-xs text-white outline-none focus:border-[#C89B3C]/40" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-mono text-[#444] uppercase tracking-wider">Author</label>
                    <input type="text" value={articleAuthor} onChange={e => setArticleAuthor(e.target.value)} placeholder="Author name"
                      className="w-full bg-white border border-[rgba(0,0,0,0.06)] rounded px-3 py-2 text-xs text-white outline-none focus:border-[#C89B3C]/40" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-mono text-[#444] uppercase tracking-wider">Reading Time (min)</label>
                    <input type="number" value={articleReadingTime} onChange={e => setArticleReadingTime(Number(e.target.value))}
                      className="w-full bg-white border border-[rgba(0,0,0,0.06)] rounded px-3 py-2 text-xs text-white outline-none focus:border-[#C89B3C]/40" />
                  </div>
                </div>
                <div className="space-y-1.5">
                  <label className="text-[11px] font-mono text-[#444] uppercase tracking-wider">Tags (comma-separated)</label>
                  <input type="text" value={articleTags} onChange={e => setArticleTags(e.target.value)} placeholder="tag1, tag2, tag3"
                    className="w-full bg-white border border-[rgba(0,0,0,0.06)] rounded px-3 py-2 text-xs text-white outline-none focus:border-[#C89B3C]/40" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-mono text-[#444] uppercase tracking-wider">Status</label>
                  ...
                  <div className="space-y-1.5 flex items-end pb-2">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" checked={articleIsFeatured} onChange={e => setArticleIsFeatured(e.target.checked)}
                        className="w-4 h-4 rounded border-neutral-700 bg-white text-[#C89B3C] focus:ring-gold-500/40 focus:ring-offset-0" />
                      <span className="text-[11px] font-mono text-[#444] uppercase tracking-wider">Featured</span>
                    </label>
                  </div>
                </div>
                <div className="space-y-1.5">
