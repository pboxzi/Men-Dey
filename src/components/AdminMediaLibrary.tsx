    ...
    <div className="space-y-6 text-left">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[rgba(0,0,0,0.06)] pb-4">
        <div>
          ...
          <p className="text-xs text-[#444] font-mono">Manage photos and videos with full CRUD.</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          {[
            ...
            <button key={tab.id} onClick={() => { setActiveMediaSubTab(tab.id); setSearchQuery(''); setCategoryFilter('ALL'); setDetailItem(null); }}
              className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded text-[10px] font-mono tracking-widest uppercase transition-all ${
                activeMediaSubTab === tab.id ? 'bg-[#C89B3C] text-neutral-950 font-bold' : 'text-[#444] hover:text-white'
              }`}
        ...
        <div className="flex-1 flex items-center gap-2 flex-wrap">
          ...
          <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)}
            className="bg-white border border-[rgba(0,0,0,0.06)] rounded-lg px-3 py-2 text-[10px] text-white font-mono outline-none focus:border-[#C89B3C]/40"
          >
            <option value="ALL">All Categories</option>
            {categories.map(c => (<option key={c.id} value={c.name}>{c.name}</option>))}
          </select>
          <div className="flex items-center gap-1 bg-white border border-[rgba(0,0,0,0.06)] rounded-lg p-0.5">
            <SortAsc className="h-3 w-3 text-[#444] ml-1.5" />
            {['sort_order', 'title', 'likes', 'created_at'].filter(s => s !== 'likes' || activeMediaSubTab === 'photos').map(s => (
              <button key={s} onClick={() => setSortBy(s)}
                className={`px-2 py-0.5 rounded text-[10px] font-mono uppercase tracking-wider transition-all ${
                  sortBy === s ? 'bg-[#C89B3C]/10 text-[#C89B3C] font-bold' : 'text-[#444] hover:text-white'
                }`}
      ...
      {loading ? (
        ...
        filteredPhotos.length === 0 ? (
          <div className="rounded-xl border border-[rgba(0,0,0,0.06)] p-12 text-center space-y-2">
            <Image className="h-8 w-8 text-[#444] mx-auto" />
            <p className="text-sm text-[#444]">No photos found.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filteredPhotos.map(photo => (
              <div key={photo.id as string} className="rounded-xl border border-[rgba(0,0,0,0.06)] bg-[#F8F6F2] overflow-hidden group hover:border-[#C89B3C]/20 transition-all">
                <div className="relative aspect-[4/3] bg-white overflow-hidden cursor-pointer" onClick={() => openDetail(photo, 'photos')}>
                  {photo.url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center"><Image className="h-8 w-8 text-[#444]" /></div>
                  )}
                ...
                <div className="p-3 space-y-1.5">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span className="shrink-0 px-1.5 py-0.5 rounded bg-white border border-[rgba(0,0,0,0.06)] text-[10px] font-mono text-[#444] uppercase">{catName(photo.category_id as number | null)}</span>
                  </div>
                  {photo.description && (
                    <p className="text-[10px] text-[#444] line-clamp-2 leading-relaxed">{photo.description as string}</p>
                  )}
                  <div className="flex items-center justify-between pt-1">
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={() => openDetail(photo, 'photos')}
                        className="p-1 rounded text-[#444] hover:text-gold-400 hover:bg-[#F8F6F2] transition-all" title="Edit"
                      ...
                      {confirmDelete === photo.id ? renderDeleteConfirm(photo.id as string, () => deletePhoto(photo.id as string)) : (
                        <button onClick={() => setConfirmDelete(photo.id as string)}
                          className="p-1 rounded text-[#444] hover:text-red-400 hover:bg-[#F8F6F2] transition-all" title="Delete"
        ...
        filteredVideos.length === 0 ? (
          <div className="rounded-xl border border-[rgba(0,0,0,0.06)] p-12 text-center space-y-2">
            <Film className="h-8 w-8 text-[#444] mx-auto" />
            <p className="text-sm text-[#444]">No videos found.</p>
          </div>
        ) : (
              ...
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="divide-y divide-neutral-900/40">
                  {filteredVideos.map(video => (
                    <tr key={video.id as string} className="hover:bg-neutral-100 transition-all">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <div className="h-7 w-7 rounded bg-white border border-[rgba(0,0,0,0.06)] flex items-center justify-center shrink-0">
                      ...
                      <td className="px-3 py-3">
                        <span className="px-1.5 py-0.5 rounded bg-white border border-[rgba(0,0,0,0.06)] text-[10px] font-mono text-[#444] uppercase">{catName(video.category_id as number | null)}</span>
                      </td>
                      <td className="px-3 py-3">
                        <span className="font-mono text-[10px] text-[#444]">{(video.youtube_id as string) || '-'}</span>
                      </td>
                      <td className="px-3 py-3">
                        <span className="font-mono text-[10px] text-[#444]">{(video.duration as string) || '-'}</span>
                      </td>
                      <td className="px-3 py-3 text-center">
                        <div className="flex items-center justify-center gap-1">
                          <button onClick={() => openDetail(video, 'videos')}
                            className="p-1.5 rounded text-[#444] hover:text-gold-400 hover:bg-[#F8F6F2] transition-all" title="Edit"
                          ><Edit3 className="h-3.5 w-3.5" /></button>
                          {confirmDelete === video.id ? renderDeleteConfirm(video.id as string, () => deleteVideo(video.id as string)) : (
                            <button onClick={() => setConfirmDelete(video.id as string)}
                              className="p-1.5 rounded text-[#444] hover:text-red-400 hover:bg-[#F8F6F2] transition-all" title="Delete"
                            ><Trash2 className="h-3.5 w-3.5" /></button>
                          )}
                        </div>
                      </td>
      ...
      <AnimatePresence>
        {detailItem && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/80 backdrop-blur-sm" onClick={() => setDetailItem(null)}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              onClick={e => e.stopPropagation()}
              className="w-full max-w-2xl bg-white border border-[rgba(0,0,0,0.06)] rounded-xl overflow-hidden shadow-2xl"
            >
              <div className="px-5 py-3.5 border-b border-[rgba(0,0,0,0.06)] flex items-center justify-between">
                <span className="text-xs font-mono font-bold tracking-widest text-[#C89B3C] uppercase">
                  <Edit3 className="h-3.5 w-3.5 inline mr-1.5" />
                  Edit {detailItem._type === 'photos' ? 'Photo' : 'Video'}
                </span>
                <button onClick={() => setDetailItem(null)} className="p-1 rounded text-[#444] hover:text-white" aria-label="Close"><X className="h-4 w-4" /></button>
              </div>

              <div className="p-5 space-y-5 max-h-[75vh] overflow-y-auto">
                {/* Preview */}
                {detailItem._type === 'photos' && detailItem.url && (
                  <div className="rounded-lg border border-[rgba(0,0,0,0.06)] overflow-hidden bg-white max-h-64 flex items-center justify-center">
                ...
                {detailItem._type === 'videos' && detailItem.youtube_id && (
                  <div className="space-y-2">
                    <div className="rounded-lg border border-[rgba(0,0,0,0.06)] overflow-hidden bg-black aspect-video">
                      <iframe
                    ...
                    <a href={`https://youtube.com/watch?v=${detailItem.youtube_id}`} target="_blank" rel="noreferrer"
                      className="flex items-center gap-1.5 text-[10px] font-mono text-[#C89B3C] hover:text-gold-400 transition-colors"
                    ><ExternalLink className="h-3 w-3" /> Open on YouTube</a>
                  </div>
                )}

                {/* ID display */}
                <div className="flex items-center gap-2 text-[11px] font-mono text-[#444]">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5 sm:col-span-2">
                    <label className="text-[11px] font-mono text-[#444] uppercase tracking-wider">Title</label>
                    <input type="text" value={editTitle} onChange={e => setEditTitle(e.target.value)}
                      className="w-full bg-white border border-[rgba(0,0,0,0.06)] rounded px-3 py-2 text-xs text-white outline-none focus:border-[#C89B3C]/40" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-mono text-[#444] uppercase tracking-wider">Category</label>
                  ...
                  {detailItem._type === 'photos' && (
                    <>
                      <div className="space-y-1.5">
                        <label className="text-[11px] font-mono text-[#444] uppercase tracking-wider">Image URL</label>
                        <input type="text" value={editUrl} onChange={e => setEditUrl(e.target.value)}
                          className="w-full bg-white border border-[rgba(0,0,0,0.06)] rounded px-3 py-2 text-xs text-white outline-none focus:border-[#C89B3C]/40" />
                      </div>
                      <div className="space-y-1.5 sm:col-span-2">
                        <label className="text-[11px] font-mono text-[#444] uppercase tracking-wider">Description</label>
                        <textarea rows={3} value={editDescription} onChange={e => setEditDescription(e.target.value)}
                          className="w-full bg-white border border-[rgba(0,0,0,0.06)] rounded px-3 py-2 text-xs text-white outline-none focus:border-[#C89B3C]/40 leading-relaxed" />
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-[11px] font-mono text-[#444] uppercase tracking-wider">Width</label>
                        <input type="number" value={editWidth} onChange={e => setEditWidth(Number(e.target.value))}
                          className="w-full bg-white border border-[rgba(0,0,0,0.06)] rounded px-3 py-2 text-xs text-white outline-none focus:border-[#C89B3C]/40" />
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-[11px] font-mono text-[#444] uppercase tracking-wider">Height</label>
                  ...
                  {detailItem._type === 'videos' && (
                    <>
                      <div className="space-y-1.5">
                        <label className="text-[11px] font-mono text-[#444] uppercase tracking-wider">YouTube ID</label>
                        <input type="text" value={editYoutubeId} onChange={e => setEditYoutubeId(e.target.value)}
                          className="w-full bg-white border border-[rgba(0,0,0,0.06)] rounded px-3 py-2 text-xs text-white outline-none focus:border-[#C89B3C]/40" />
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-[11px] font-mono text-[#444] uppercase tracking-wider">Duration</label>
              ...
              {/* Footer */}
              <div className="px-5 py-3.5 border-t border-[rgba(0,0,0,0.06)] flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setDetailItem(null)}
                    className="px-4 py-1.5 rounded-lg border border-[rgba(0,0,0,0.06)] text-[#444] hover:text-white text-[10px] font-mono uppercase tracking-widest transition-all"
                  >Cancel</button>
      ...
      <AnimatePresence>
        {showAddModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/80 backdrop-blur-sm" onClick={() => setShowAddModal(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              onClick={e => e.stopPropagation()}
              className="w-full max-w-lg bg-white border border-[rgba(0,0,0,0.06)] rounded-xl overflow-hidden shadow-2xl"
            >
              <div className="px-5 py-3.5 border-b border-[rgba(0,0,0,0.06)] flex items-center justify-between">
                <span className="text-xs font-mono font-bold tracking-widest text-[#C89B3C] uppercase">
                ...
                <button onClick={() => setShowAddModal(false)} className="p-1 rounded text-[#444] hover:text-white" aria-label="Close"><X className="h-4 w-4" /></button>
              </div>
              ...
              <form onSubmit={activeMediaSubTab === 'photos' ? addPhoto : addVideo} className="p-5 space-y-4">
                ...
                <div className="flex gap-2 justify-end pt-2">
                  <button type="button" onClick={() => setShowAddModal(false)}
                    className="px-4 py-2 border border-[rgba(0,0,0,0.06)] rounded-lg text-[10px] font-mono text-[#444] hover:text-white uppercase tracking-widest transition-all"
                  >Cancel</button>
