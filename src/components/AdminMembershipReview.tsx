...
const STATUS_STYLES: Record<string, string> = {
  ...
  cancelled: 'bg-[#F8F6F2] text-[#444] border-neutral-700',
};
    ...
    <div className="space-y-6 text-left">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[rgba(0,0,0,0.06)] pb-4">
        <div>
          ...
          <p className="text-xs text-[#444] font-mono">Review, approve, and manage all membership applications.</p>
        </div>
      ...
      <div className="grid grid-cols-5 gap-3">
        {[
          ...
          { label: 'Closed', value: stats.cancelled, color: 'text-[#444]' },
        ].map(s => (
          ...
          {FILTER_TABS.map(tab => (
            <button key={tab.key} onClick={() => setFilterTab(tab.key)}
              className={`px-3 py-1.5 rounded text-[10px] font-mono tracking-widest uppercase transition-all ${
                filterTab === tab.key ? 'bg-[#C89B3C] text-neutral-950 font-bold' : 'text-[#444] hover:text-white'
              }`}
            >{tab.label}</button>
          ))}
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-[rgba(0,0,0,0.06)] p-12 text-center space-y-2">
          <Crown className="h-8 w-8 text-[#444] mx-auto" />
          <p className="text-sm text-[#444]">No membership requests found.</p>
          <p className="text-[10px] font-mono text-[#444]">Try a different filter or search term.</p>
        </div>
      ) : (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {filtered.map(req => (
            <div key={req.id} onClick={() => openDetail(req)}
              className="rounded-xl border border-[rgba(0,0,0,0.06)] bg-white hover:border-[rgba(0,0,0,0.06)] transition-all cursor-pointer p-4 space-y-3 group"
            >
              {/* Top row */}
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-2.5">
                  <div className="h-9 w-9 rounded-full bg-white border border-[rgba(0,0,0,0.06)] flex items-center justify-center overflow-hidden shrink-0">
                    {req.profile_photo ? (
                      <img src={req.profile_photo} alt={req.full_name || "Member profile"} loading="lazy" className="h-full w-full object-cover" />
                    ) : (
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[11px] text-[#444] font-mono truncate">{req.member_email}</p>
                  </div>
                </div>
                <span className={`shrink-0 inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-mono font-bold uppercase border ${STATUS_STYLES[req.status] || 'bg-white text-[#444] border-[rgba(0,0,0,0.06)]'}`}>
              ...
              <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 text-[10px]">
                <div><span className="text-[#444]">Tier</span><p className="text-[#C89B3C] font-bold truncate">{req.tier_name || '-'}</p></div>
                <div><span className="text-[#444]">Serial</span><p className="text-white font-mono truncate">{req.card_serial || '-'}</p></div>
                {req.membership_number && <div className="col-span-2"><span className="text-[#444]">Membership #</span><p className="text-white font-mono">{req.membership_number}</p></div>}
                <div><span className="text-[#444]">Submitted</span><p className="text-white">{new Date(req.created_at).toLocaleDateString()}</p></div>
                <div><span className="text-[#444]">Comm</span><p className="text-white capitalize">{req.comm_method || '-'}</p></div>
              </div>

              {/* Active membership extras */}
              {req.status === 'active' && (
                <div className="flex items-center gap-3 text-[11px] font-mono text-[#444] pt-1.5 border-t border-[rgba(0,0,0,0.06)]/60">
              ...
              <div className="flex items-center justify-end gap-1 pt-1 border-t border-[rgba(0,0,0,0.06)]/60 text-[11px] font-mono uppercase tracking-wider text-[#444] group-hover:text-gold-400 transition-all duration-300 group-hover:gap-2">
        ...
        {showDetail && selectedRequest && (
          ...
          >
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              ...
              className="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-2xl border border-[rgba(0,0,0,0.06)] bg-white p-6 shadow-2xl space-y-5"
            >
              {/* Modal Header */}
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  <div className="h-12 w-12 rounded-full bg-white border border-[rgba(0,0,0,0.06)] flex items-center justify-center overflow-hidden">
                    {selectedRequest.profile_photo ? (
                      <img src={selectedRequest.profile_photo} alt={selectedRequest.full_name || "Member profile"} loading="lazy" className="h-full w-full object-cover" />
                    ) : (
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#444] font-mono">{selectedRequest.member_email}</p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-3">
                  <h4 className="text-[11px] font-mono text-[#C89B3C] uppercase tracking-widest font-bold">Application Details</h4>
                  <div className="space-y-2 text-[11px]">
                    {[
                      ['Tier', selectedRequest.tier_name, 'text-[#C89B3C] font-bold'],
                      ...
                      <div key={String(l)} className="flex justify-between">
                        <span className="text-[#444]">{String(l)}</span>
                ...
                <div className="space-y-3">
                  <h4 className="text-[11px] font-mono text-[#C89B3C] uppercase tracking-widest font-bold">Member Info</h4>
                  <div className="space-y-2 text-[11px]">
                    {[
                    ...
                    <div className="pt-2 space-y-2">
                      ...
                      <div className="space-y-2 text-[11px]">
                        <div className="flex justify-between">
                          <span className="text-[#444]">Activated</span>
                        ...
                        <div className="flex justify-between">
                          <span className="text-[#444]">Expires</span>
              ...
              {selectedRequest.cancel_reason && (
                <div className="rounded-lg border border-red-500/20 bg-red-500/[0.02] p-3 space-y-1">
                  ...
                  <p className="text-[11px] text-[#444]">{selectedRequest.cancel_reason}</p>
                </div>
              )}

              {/* Admin Notes */}
              <div className="space-y-2">
                <h4 className="text-[11px] font-mono text-[#444] uppercase tracking-widest font-bold">Admin Notes</h4>
                {selectedRequest.admin_notes ? (
                  <p className="text-[11px] text-[#444] whitespace-pre-line bg-[#F8F6F2] border border-[rgba(0,0,0,0.06)] rounded-lg p-3">{selectedRequest.admin_notes}</p>
                ) : (
                  <p className="text-[10px] text-[#444] italic">No notes yet.</p>
                )}
                <textarea value={adminNote} onChange={e => setAdminNote(e.target.value)}
                  placeholder="Add a note..."
                  className="w-full bg-white border border-[rgba(0,0,0,0.06)] rounded-lg px-3 py-2 text-xs text-white outline-none focus:border-[#C89B3C]/40 resize-none" rows={2}
                />
              </div>

              {/* Action Buttons */}
              <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-[rgba(0,0,0,0.06)]/60">
                {(selectedRequest.status === 'pending' || selectedRequest.status === 'upgrade_pending') && (
                  <>
                    ...
                    <span className="h-6 w-px bg-[#F8F6F2]/60" />
                    <a href={`mailto:${selectedRequest.member_email}`}
                      className="group/btn flex items-center gap-2 px-4 py-2.5 rounded-xl border border-neutral-700/40 bg-[#F8F6F2]/20 text-[#444] hover:bg-neutral-700 hover:text-white text-[10px] font-mono uppercase tracking-widest active:scale-[0.97] transition-all duration-200"
                ...
                <div className="flex items-center gap-2 ml-auto">
                  {confirmDelete === selectedRequest.id ? (
                    <div className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-red-500/10 border border-red-500/30">
                      ...
                      <button onClick={() => setConfirmDelete(null)}
                        className="px-2.5 py-1 rounded-lg border border-neutral-700 text-[#444] hover:text-white text-[11px] font-mono uppercase transition-all"
                      >No</button>
                    </div>
                  ) : (
                  ...
                  <button onClick={() => setShowDetail(false)}
                    className="group/btn flex items-center gap-2 px-5 py-2.5 rounded-xl border border-[rgba(0,0,0,0.06)] bg-[#F8F6F2] text-[#444] hover:bg-[#F8F6F2] hover:text-white text-[10px] font-mono uppercase tracking-widest active:scale-[0.97] transition-all duration-200"
                  >Close</button>
                </div>
              </div>
            </motion.div>
