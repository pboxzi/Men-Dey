    ...
    <div className="space-y-6 text-left">
      <div className="space-y-1 border-b border-[rgba(0,0,0,0.06)] pb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-serif text-xl font-bold tracking-wider text-[#111] uppercase">Messages</h2>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 rounded-xl border border-[rgba(0,0,0,0.06)] bg-white overflow-hidden min-h-[500px]">
        ...
        <div className="lg:col-span-1 border-b lg:border-b-0 lg:border-r border-[rgba(0,0,0,0.06)] flex flex-col">
          <div className="p-3 border-b border-[rgba(0,0,0,0.06)] flex justify-between items-center">
            <p className="text-[10px] font-mono text-[#444] uppercase tracking-widest">Conversations</p>
            <button
          ...
          {showNewChat && (
            <div className="p-3 border-b border-[rgba(0,0,0,0.06)] space-y-2">
              <input
                ...
                className="w-full bg-white border border-[rgba(0,0,0,0.06)] rounded-lg px-3 py-2 text-[11px] text-[#111] outline-none focus:border-[#C89B3C]/40"
                autoFocus
                onKeyDown={e => e.key === 'Enter' && handleCreateConv()}
              />
              <div className="flex gap-2">
                <button onClick={handleCreateConv}
                  className="flex-1 py-1.5 rounded bg-[#C89B3C] text-neutral-950 text-[11px] font-mono font-bold uppercase">Start</button>
          ...
          <div className="flex-1 overflow-y-auto">
            {conversations.length === 0 ? (
              <div className="p-6 text-center">
                <MessageSquare className="h-6 w-6 text-[#444] mx-auto mb-2" />
                <p className="text-[10px] text-[#444]">No conversations yet</p>
              </div>
            ) : (
              conversations.map(conv => (
                ...
                >
                  <p className="text-[11px] font-bold text-[#111] truncate">{conv.subject}</p>
          ...
          {!selectedConv ? (
            <div className="flex-1 flex items-center justify-center p-8">
              <div className="text-center space-y-3">
                ...
                <p className="text-sm text-[#444]">Select or start a conversation</p>
                <p className="text-[10px] text-[#444] font-mono">Click "New" to message the admin team</p>
              </div>
            </div>
          ) : (
            <>
              ...
              <div className="px-4 py-3 border-b border-[rgba(0,0,0,0.06)] flex items-center gap-3">
                <div className="h-8 w-8 rounded-full bg-white border border-[rgba(0,0,0,0.06)] flex items-center justify-center">
                  <span className="text-[11px] font-mono font-bold text-[#C89B3C]">GA</span>
                </div>
                <div className="flex-1">
                  <p className="text-xs font-bold text-[#111]">{selectedConv.subject}</p>
              ...
              <div ref={scrollContainerRef} onScroll={handleScroll} className="flex-1 overflow-y-auto px-4 py-4 space-y-4 min-h-[300px]">
                {messages.length === 0 && (
                  <div className="flex items-center justify-center h-full">
                    <p className="text-[10px] text-[#444] font-mono">No messages yet. Say hello!</p>
                  </div>
                )}
                {messages.map(msg => (
