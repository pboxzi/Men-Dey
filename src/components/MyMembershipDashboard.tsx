...
export default function MyMembershipDashboard({ userId, authName, rank, progressPercent, content }: Props) {
  ...
  const getTierStyle = (tierId: string) => {
    ...
    return {
      bg_color: t?.bg_color || 'from-neutral-900 via-neutral-50 to-white',
      border_color: t?.border_color || 'border-[rgba(0,0,0,0.06)]',
      icon_color: t?.icon_color || 'text-[#444]',
    };
  };

  if (loading) return <div className="flex items-center justify-center py-20"><Loader2 className="h-6 w-6 text-[#C89B3C] animate-spin" /></div>;
    ...
    return (
      <div className="space-y-6 text-left">
        <div className="space-y-1 border-b border-[rgba(0,0,0,0.06)] pb-4">
          <h2 className="font-serif text-xl font-bold tracking-wider text-[#111] uppercase">Membership Dashboard</h2>
          <p className="text-xs text-[#444] font-mono">Your Sanctuary membership status</p>
        </div>
        <div className="rounded-xl border border-[rgba(0,0,0,0.06)] bg-white p-12 text-center space-y-4">
          <Crown className="h-10 w-10 text-[#444] mx-auto" />
          <h3 className="font-serif text-lg font-bold text-[#111]">You don't have a membership yet.</h3>
          <p className="text-xs text-[#444]">Explore the membership tiers and apply to become an official member.</p>
          <button onClick={() => navigate('/membership')}
            className="bg-[#C89B3C] hover:bg-[#A97828] text-neutral-950 font-bold py-2.5 px-6 rounded tracking-widest uppercase text-xs transition-all"
  ...
  if (membership.status === 'pending' || membership.status === 'upgrade_pending') {
    ...
    return (
      <div className="space-y-6 text-left">
        ...
        <div className="rounded-xl border border-amber-500/20 bg-white p-6 space-y-5">
          <div className="flex items-center gap-3">
            <Clock className="h-8 w-8 text-amber-500" />
            <div>
              <h3 className="font-bold text-[#111]">{isUpgrade ? 'Upgrade Request Submitted' : 'Membership Request Submitted'}</h3>
              <p className="text-xs text-[#444]">Submitted {new Date(membership.created_at).toLocaleDateString()}</p>
            </div>
          </div>
          ...
          <div className={`relative w-full max-w-[340px] mx-auto aspect-[1.58/1] rounded-2xl border bg-gradient-to-br ${style.bg_color} ${style.border_color} p-5 flex flex-col justify-between overflow-hidden shadow-xl`}>
            <div className="absolute right-0 top-0 h-40 w-40 rounded-full bg-[#C89B3C]/5 blur-[50px] pointer-events-none" />
            <div className="flex justify-between items-start z-10">
              <div className="flex items-center gap-1.5">
                <span className="font-serif text-base font-bold tracking-widest text-white">GA</span>
                <div className="h-3 w-[1px] bg-[#F8F6F2]" />
                <div className="flex flex-col">
                  <span className="font-serif text-[11px] font-bold tracking-wider text-[#444]">GILLIAN ANDERSON</span>
                  <span className="font-mono text-[5px] tracking-[0.2em] text-[#C89B3C]">OFFICIAL SANCTUARY</span>
                </div>
              </div>
              <span className="font-mono text-[10px] font-bold text-[#444] tracking-wider">MEMBER CARD</span>
            </div>
            <div className="flex gap-4 items-center z-10 pt-4">
              <div className="h-14 w-14 rounded-full border border-[rgba(0,0,0,0.06)] bg-white overflow-hidden flex items-center justify-center shrink-0">
                {membership.profile_photo ? <img src={membership.profile_photo} alt="Member profile photo" loading="lazy" className="h-full w-full object-cover" /> : <User className="h-6 w-6 text-[#444]" />}
              </div>
              <div className="space-y-1 text-left">
                <span className="text-[10px] font-mono text-[#444] uppercase">OFFICIAL MEMBER</span>
                <h5 className="font-serif text-sm font-bold text-[#111] tracking-wide truncate max-w-[180px]">{membership.card_name || 'Member'}</h5>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-mono text-[#444]">PENDING</span>
                </div>
              </div>
            </div>
            <div className="flex justify-between items-end border-t border-[rgba(0,0,0,0.06)] pt-3 z-10">
              <div className="text-left font-mono text-[10px] text-[#444]">
                <span className="block">SERIAL NUMBER</span>
                <span className="font-semibold text-[#444]">{membership.card_serial}</span>
              </div>
          ...
          <div className="border border-[rgba(0,0,0,0.06)] rounded-xl bg-[#F8F6F2] p-4 space-y-2">
            <div className="flex justify-between text-xs"><span className="text-[#444]">Tier</span><span className="text-[#C89B3C] font-bold">{membership.tier_name}</span></div>
            <div className="flex justify-between text-xs"><span className="text-[#444]">Card Name</span><span className="text-[#111]">{membership.card_name}</span></div>
            <div className="flex justify-between text-xs"><span className="text-[#444]">Serial</span><span className="text-[#333] font-mono text-[10px]">{membership.card_serial}</span></div>
            <div className="flex justify-between text-xs"><span className="text-[#444]">Communication</span><span className="text-[#111] capitalize">{membership.comm_method}</span></div>
          </div>
        </div>
      </div>
    );
        ...
        <div className={`relative w-full max-w-[380px] mx-auto aspect-[1.58/1] rounded-2xl border-2 bg-gradient-to-br ${style.bg_color} ${style.border_color} p-5 md:p-6 flex flex-col justify-between overflow-hidden shadow-2xl`}>
          ...
          <div className="flex justify-between items-start z-10">
            <div className="flex items-center gap-1.5">
              ...
              <div className="h-4 w-[1px] bg-[#F8F6F2]" />
              <div className="flex flex-col">
            ...
            <Crown className="h-5 w-5 text-[#C89B3C]" />
          </div>
          <div className="flex gap-4 items-center z-10 pt-4">
            <div className="h-16 w-16 rounded-full border-2 border-[#C89B3C]/40 bg-white overflow-hidden flex items-center justify-center shrink-0 shadow-lg shadow-gold-500/10">
{membership.profile_photo ? <img src={membership.profile_photo} alt="Member profile photo" loading="lazy" className="h-full w-full object-cover" /> : <User className="h-7 w-7 text-[#444]" />}
            </div>
            <div className="space-y-1 text-left">
                <span className="text-[10px] font-mono text-[#444] uppercase">OFFICIAL MEMBER</span>
                <h5 className="font-serif text-base font-bold text-[#111] tracking-wide truncate max-w-[200px]">{membership.card_name || 'Member'}</h5>
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 border-t border-[rgba(0,0,0,0.06)]/60 pt-3 z-10 text-left">
            <div><span className="text-[10px] font-mono text-[#444] uppercase block">Membership #</span><span className="text-[10px] font-mono font-bold text-[#444]">{membership.membership_number || 'N/A'}</span></div>
            <div><span className="text-[10px] font-mono text-[#444] uppercase block">Serial</span><span className="text-[10px] font-mono font-bold text-[#444]">{membership.card_serial}</span></div>
            <div><span className="text-[10px] font-mono text-[#444] uppercase block">Activated</span><span className="text-[10px] font-mono font-bold text-[#444]">{membership.activation_date ? new Date(membership.activation_date).toLocaleDateString() : 'N/A'}</span></div>
            <div><span className="text-[10px] font-mono text-[#444] uppercase block">Expires</span><span className="text-[10px] font-mono font-bold text-[#444]">{membership.expiration_date ? new Date(membership.expiration_date).toLocaleDateString() : 'Never'}</span></div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-[rgba(0,0,0,0.06)] bg-white p-5 space-y-3">
          <h3 className="text-xs font-mono font-bold text-[#111] uppercase tracking-widest flex items-center gap-2">
            <Crown className="h-3.5 w-3.5 text-[#C89B3C]" /> Your Benefits
          </h3>
        ...
        <div className="flex flex-wrap gap-3">
          <button onClick={downloadCard}
            className="border border-[rgba(0,0,0,0.06)] hover:border-[#C89B3C]/40 text-[#444] hover:text-[#111] font-bold py-2.5 px-5 rounded text-xs transition-all flex items-center gap-1.5">
            <Download className="h-3.5 w-3.5 text-[#C89B3C]" /> Download Membership Card
          </button>
          {higherTiers.length > 0 && (
            <button onClick={() => { setUpgradeTier(''); setUpgradeCommMethod(null); setShowUpgradeModal(true); }}
              className="border border-[#C89B3C]/40 hover:bg-[#C89B3C]/10 text-[#C89B3C] font-bold py-2.5 px-5 rounded text-xs transition-all flex items-center gap-1.5">
          ...
          {showUpgradeModal && (
            ...
            >
              <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
                className="w-full max-w-md rounded-xl border border-[rgba(0,0,0,0.06)] bg-white p-6 space-y-5 shadow-2xl"
              >
                <div className="text-center space-y-1">
                  <h3 className="font-serif text-lg font-bold text-[#111]">Upgrade Membership</h3>
                  <p className="text-xs text-[#444]">Choose your new tier. You will be upgraded from <span className="text-[#C89B3C]">{membership.tier_name}</span>.</p>
                </div>
                <div className="space-y-3">
                  {higherTiers.map((t) => (
                    <button key={t.id} onClick={() => setUpgradeTier(t.id)}
                      className={`w-full flex items-center justify-between p-4 rounded-lg border transition-all text-left ${upgradeTier === t.id ? 'border-[#C89B3C]/50 bg-[#C89B3C]/5' : 'border-[rgba(0,0,0,0.06)] hover:border-[rgba(0,0,0,0.06)] bg-white'}`}
                    >
                      <div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-3">
                      <button onClick={() => setUpgradeCommMethod('whatsapp')}
                        className={`flex-1 flex flex-col items-center gap-2 p-4 rounded-lg border transition-all ${upgradeCommMethod === 'whatsapp' ? 'border-emerald-500/50 bg-emerald-500/5' : 'border-[rgba(0,0,0,0.06)] hover:border-[rgba(0,0,0,0.06)] bg-white'}`}
                      >
                        <MessageCircle className={`h-5 w-5 ${upgradeCommMethod === 'whatsapp' ? 'text-emerald-500' : 'text-[#444]'}`} />
                        <span className="text-[10px] font-mono text-[#444]">WhatsApp</span>
                      </button>
                      <button onClick={() => setUpgradeCommMethod('email')}
                        className={`flex-1 flex flex-col items-center gap-2 p-4 rounded-lg border transition-all ${upgradeCommMethod === 'email' ? 'border-[#C89B3C]/50 bg-[#C89B3C]/5' : 'border-[rgba(0,0,0,0.06)] hover:border-[rgba(0,0,0,0.06)] bg-white'}`}
                      >
                ...
                <div className="flex gap-3">
                  <button onClick={() => setShowUpgradeModal(false)}
                    className="flex-1 py-2.5 rounded border border-[rgba(0,0,0,0.06)] text-[#444] text-xs font-mono font-bold uppercase tracking-widest hover:text-neutral-900 hover:border-neutral-300 transition-all">Cancel</button>
                  <button onClick={handleUpgrade} disabled={!upgradeTier || !upgradeCommMethod || upgrading}
                    className="flex-1 py-2.5 rounded bg-[#C89B3C] text-neutral-950 text-xs font-mono font-bold uppercase tracking-widest hover:bg-[#A97828] transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                  >
        ...
        <AnimatePresence>
          {toast && (
            <motion.div initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.95 }}
              className="fixed bottom-4 right-4 z-50 flex items-center gap-2.5 rounded-lg border border-[#C89B3C] bg-white px-4 py-3 shadow-2xl shadow-gold-500/10 min-w-[300px]"
            >
              <div className="h-2 w-2 rounded-full bg-[#C89B3C] animate-pulse" />
              <div className="flex-1 text-xs text-left">
                <p className="font-mono text-[#C89B3C] uppercase tracking-widest font-bold text-[11px]">SYSTEM MSG</p>
  ...
  if (membership.status === 'expired') {
    return (
      <div className="space-y-6 text-left">
        ...
        <div className="rounded-xl border border-red-500/20 bg-white p-6 text-center space-y-4">
          ...
          <p className="text-xs text-[#444]">Your membership has expired. Renew to regain access to benefits.</p>
          <button onClick={() => navigate('/')} className="bg-[#C89B3C] hover:bg-[#A97828] text-neutral-950 font-bold py-2.5 px-6 rounded tracking-widest uppercase text-xs transition-all">Renew Membership</button>
        </div>
      </div>
    );
  }
  ...
  if (membership.status === 'cancelled') {
    return (
      <div className="space-y-6 text-left">
        ...
        <div className="rounded-xl border border-[rgba(0,0,0,0.06)] bg-white p-6 text-center space-y-4">
          <ShieldAlert className="h-10 w-10 text-[#444] mx-auto" />
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-[rgba(0,0,0,0.06)] bg-white text-[#444] text-[10px] font-mono"><XCircle className="h-3 w-3" /> Cancelled</div>
          {membership.cancel_reason && <p className="text-xs text-[#444]">Reason: {membership.cancel_reason}</p>}
          <button onClick={() => navigate('/')} className="border border-[rgba(0,0,0,0.06)] hover:border-[#C89B3C]/40 text-[#444] hover:text-white font-bold py-2.5 px-6 rounded tracking-widest uppercase text-xs transition-all">Reapply</button>
        </div>
      </div>
    );
  }
